...
def rtlpower(# Mendefinisikan Parameter yang Digunakan
    LOWER_BAND = '455525000',         # Batas Bawah Frekuensi dengan satuan MHz
    UPPER_BAND = '457525000',        # Batas Atas Frekuensi dengan satuan MHz
    ...
    command = f"rtl_power -f {LOWER_BAND}:{UPPER_BAND}:{BIN_SIZE} -g {GAIN}{DIRECT_SAMPLING}{PEAK_HOLD} -p 0 -1 -i {INTERVAL} -d 0 {OUTPUT_FILE}"

    subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)

def rtlpower2(# Mendefinisikan Parameter yang Digunakan
    LOWER_BAND = '455525000',         # Batas Bawah Frekuensi dengan satuan MHz
    UPPER_BAND = '457525000',        # Batas Atas Frekuensi dengan satuan MHz
    BIN_SIZE = '100000',             # Bin size for frequency resolution
    INTERVAL = '1',                 # Interval 
    GAIN = '0',                      # Besar Gain
    OUTPUT_FILE = 'fm_spectrum2.csv', # Path File Output 
    CORRECTION = '0%',               # 
    DIRECT = 'NO',                   # Mengaktifkan Direct Sampling
    PEAK = 'NO'                     # Mengaktifkan Peak Hold
    ):                   
    ...
    command = f"rtl_power -f {LOWER_BAND}:{UPPER_BAND}:{BIN_SIZE} -g {GAIN}{DIRECT_SAMPLING}{PEAK_HOLD} -p 0 -1 -i {INTERVAL} -d 1 {OUTPUT_FILE}"
...
def data_json(file_path="fm_spectrum.csv", json_path="data.json"):
    last_size = 0
    temp_value = 0
    while True:
        try:
...
def data_json2(file_path="fm_spectrum2.csv", json_path="data2.json"):
    last_size = 0
    temp_value = 0
    while True:
        try:
...
if __name__ == "__main__" :
    ...
    thread3 = threading.Thread(target=data_json2)
    thread4 = threading.Thread(target=run_rtlpower2, daemon=True)

    # Menjalankan server pada thread baru
